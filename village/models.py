...
def medical_ninja_validator(id_value):
    ...
    print(id_value, ninja, techniques)
    for t in techniques:
        for ct in CurativeTechnique.objects.all():
            if t.id == ct.technique.id:
                return id_value
...
class Parchment(models.Model):
    ...
    def __str__(self):
        return 'Seal made by {0} '.format(self.ninja.person.name_p) + 'with technique {0} '.format(str(self.technique.name)) + '({0})'.format(self.ninja.person.id)
...
class Client(models.Model):
    ...
    def __str__(self):
        return self.name_c + '({0})'.format(self.id)
...
class Team(models.Model):
    ...
    members = models.ManyToManyField(Ninja, blank=False)
    def __str__(self):
...
class Mission(models.Model):
    captain = models.ForeignKey(Jounin, null=False, blank=False, on_delete=models.CASCADE)
    ...
    parchments = models.ManyToManyField(Parchment, blank=True)
